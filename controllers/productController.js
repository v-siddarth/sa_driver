...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    cb(null, Date.now() + path.extname(file.originalname)); // Rename the file with a timestamp to avoid name conflicts
  },
});

const upload = multer({ storage: storage }).single("productImage"); // 'productImage' is the name of the field in the form
...
const productCreate = (req, res) => {
  ...
  upload(req, res, async (err) => {
    ...
    try {
      const productData = {
        ...req.body,
        // seller: req.user.userId, // Ensure seller ID is stored in the product

        productImage: req.file ? `/uploads/${req.file.filename}` : "", // Store the file path in the database
      };
